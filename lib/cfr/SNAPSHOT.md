...
1. An email arrives from eCFR. Subscriptions are active for Parts 4, 5, 7 and
   ...
   from that date.
2. Re-retrieve the affected part from the URL recorded in the file header.
3. Replace the file and commit. `git diff` shows exactly which sentences moved.
4. Review the diff against the corresponding prompt in `lib/`. Any check citing
   a changed section needs revisiting.
5. Update the retrieval date and "up to date as of" date in the table above.

## Known history worth remembering

Parts 5 and 7 were comprehensively restructured and renumbered by
**T.D. TTB-176, Feb. 9, 2022**. The document spans both parts but they begin on
different Federal Register pages: Part 5 at 87 FR 7579, Part 7 at 87 FR 7605.
Section numbers from before that date do not map onto the current text — some
were renumbered, others no longer exist. Any citation to Part 5 or Part 7 that
predates February 2022 should be treated as suspect until verified against this
corpus.

## Known defects in the source

These are errors in eCFR's published data, reproduced here deliberately. The
files are meant to match what the government served, defects included —
silently correcting upstream errors would make every future `git diff` noisy
and would break the guarantee that this corpus equals its source.

- **Part 5, source line.** Reads `T.D. TTB-176, 87 7579, Feb. 9, 2022` — the
  "FR" is missing. The omission is present in the eCFR XML as delivered. The
  correct citation is 87 FR 7579. Part 7's equivalent line is not affected.

If eCFR corrects one of these, the fix will appear in the diff on the next pull
and this note should be removed.
